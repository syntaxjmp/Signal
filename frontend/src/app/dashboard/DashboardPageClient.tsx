...
export default function DashboardPage() {
  ...
  const runProjectScan = useCallback(
    async (p: Project, openInNewTab: boolean) => {
      ...
      try {
        ...
        if (json?.scanId) {
          ...
          await loadProjects({ silent: true });
          const url = `/findingsreport/${p.id}?scanId=${encodeURIComponent(String(json.scanId))}`;
          if (openInNewTab) {
                        ...
                        <div className="dash-project-card__layout">
                          <div className="dash-project-card__btnRow">
                            ...
                            <button
                              ...
                              onClick={(e) => {
                                void runProjectScan(p, e.ctrlKey || e.metaKey);
                              }}
                              onAuxClick={(e) => {
                                if (e.button === 1) {
                                  e.preventDefault();
                                  void runProjectScan(p, true);
                                }
                              }}
                              title="Tip: Ctrl+click or middle-click to open the findings report in a new tab when the scan finishes"
                            >
                              {scanBusy[p.id] ? "Scanning..." : "Run scan"}
                            </button>
                            {p.latestScanStatus && p.latestScanStatus !== "not started" && (
                            <Link href={`/findingsreport/${p.id}`} className="dash-btn dash-btn--secondary">
                              View findings
                            </Link>
                            )}
                            <Link href={`/compliance/${p.id}`} className="dash-btn dash-btn--secondary">
                              Compliance
                            </Link>
                            <button
                              type="button"
